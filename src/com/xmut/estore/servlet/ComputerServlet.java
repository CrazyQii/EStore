...
import com.xmut.estore.domain.*;
import com.xmut.estore.service.CommentService;
...
public class ComputerServlet extends HttpServlet {
	...
	protected void updateStock(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
		String idStr = request.getParameter("id");
		int id = -1;
		...
		computer.setStoreNumber(storeNumber);
		computerService.updateStock(id,storeNumber);
		
		//5. 传回 JSON 数据: brand, storeNumber
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("brand", computer.getBrand()+computer.getModel());
		result.put("storeNumber", computer.getStoreNumber());
	...
	protected void getComments(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
		...
		List<Comment> commentList = commentService.getComment(computerId);
		Collections.reverse(commentList);
	...
	protected void addComment(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		comment.setComputerId(computerId);
		commentDAO.addComment(comment);
		response.sendRedirect("computerServlet?method=getComputer&id="+ computerId);
	}
}
